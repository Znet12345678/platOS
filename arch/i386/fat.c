...
fs_t *fat_init(dev_t *fpart){
	...
	int r = ((int(*)())fpart->read)(fpart,head,0,getATA()->clustersize);
	...
	first_data_sector = bpb->numResv + (bpb->spf16*fat_size)+root_dir_sectors;
	first_fat_sector = bpb->numResv;
	data_sectors = bpb->tsect - (bpb->numResv+(bpb->spf16*fat_size));
	total_clusters = data_sectors/bpb->spc;
	if(total_clusters < 4085)
		bits = 12;
	else if(total_clusters < 6625)
		bits = 16;
